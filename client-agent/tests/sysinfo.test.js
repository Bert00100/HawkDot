...
import { parseGateway, parseResolvConf, collectIdentity, collectSnapshot } from '../lib/sysinfo.js';
...
test('collectIdentity: monta identidade com os campos novos', async () => {
  const id = await collectIdentity('Maquina X');
  assert.ok(id.agent_id.startsWith('hd-'));
  assert.equal(id.agent_name, 'Maquina X');
  assert.ok(id.arch, 'deve ter arch');
  assert.ok(typeof id.hostname === 'string');
  // serial/model/gateway/dns podem ser null em ambientes sem DMI/rede, mas as chaves existem
