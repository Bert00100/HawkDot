...
//
// Recebe o payload de uma coleta vinda do agente e grava em 4 tabelas
  ...
  const sql = `
    INSERT INTO agents
      (agent_id, agent_name, hostname, os, os_version, arch, model, serial_number,
       mac_addresses, local_ips, dns_servers, default_gateway, public_ip)
...
async function insertTestResults(client, collectionId, tests = []) {
  for (const t of tests) {
    await client.query(
      ...
      [
        collectionId,        t.type,                    t.name                ?? null,
        t.target             ?? null,  t.success          ?? null,
        t.latency_ms         ?? null,  t.packet_loss_percent ?? null, t.jitter_ms ?? null,
        t.response_time_ms   ?? null,  t.resolved_address ?? null,
        t.total_time_ms      ?? null,  t.http_status_code ?? null,
        t.throughput_mbps    ?? null,  t.bytes_transferred ?? null,   t.speed_kind ?? null,
...
async function insertSnapshot(client, collectionId, s) {
  ...
  await client.query(
    ...
    [
      collectionId,
      s.cpu_model           ?? null, s.cpu_cores            ?? null,
      s.cpu_logical_cores   ?? null, s.cpu_load_1           ?? null,
      s.memory_physical_bytes ?? null, s.memory_used_bytes  ?? null,
      s.memory_free_bytes   ?? null, s.memory_alloc_bytes   ?? null,
      s.disk_total_bytes    ?? null, s.disk_free_bytes      ?? null,
      s.temperature_c       ?? null, s.temp_source          ?? null,
      s.public_ip           ?? null, s.isp                  ?? null,
      s.interface_name      ?? null, s.interface_status     ?? null,
